...
const PatientAppointments = () => {
    ...
    const fetchData = async () => {
      ...
      try {
        const [apptsRes, docsRes] = await Promise.all([
          axios.get(`${API_URL}/api/appointments/patient/${user._id}`, {
            headers: { Authorization: `Bearer ${localStorage.getItem('token')}` }
          }),
          axios.get(`${API_URL}/api/auth/doctors`, {
            headers: { Authorization: `Bearer ${localStorage.getItem('token')}` }
          })
        ]);
  ...
  const fetchAvailableTimes = async (doctorId, date) => {
    ...
    try {
      const res = await axios.get(`${API_URL}/api/appointments/availability/${doctorId}`, {
        headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
        params: { date }
      });
